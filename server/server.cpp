...
void StunServer::run() {
  ...
  while (true) {
    ...
    try {
      const stun::StunMessage request = stun::StunMessage::deserialize(received_packet);
      ...
      if (request.header.cclass != stun::StunClass::request) {
        ...
        send_message(create_error_response(request, kBadRequestErrorCode,
                                           "Bad Request"),
                                           client_address, client_address_size);
        continue;
      }
      send_message(create_binding_response(request, client_address), client_address, client_address_size);
...
StunMessage StunServer::create_binding_response(const StunMessage &request,
  ...
  response.header.method = StunMethod::binding;
  response.header.cclass = StunClass::success_response;
  response.header.message_length = 0;
  response.header.magic_cookie = kMagicCookie;
  ...
  if (m_xor_mapped_mode) {
    ...
    std::make_unique<XorMappedAddressAttribute>(XorMappedAddressAttribute::from_ip_address(
                                                client_port, IpAddress::from_ipv4(client_ip),
                                                response.header.magic_cookie)));
  } 
  else {
    response.attributes.push_back(std::make_unique<stun::MappedAddressAttribute>(
                                  MappedAddressAttribute::from_ip_address(client_port,
                                                                          stun::IpAddress::from_ipv4(client_ip))));
  }
  return response;
}

StunMessage StunServer::create_error_response(const StunMessage &request,
                                              uint16_t error_code,
                                              const std::string &reason_phrase) {
  StunMessage response{};
  response.header.method = request.header.method;
  response.header.cclass = StunClass::error_response;
  response.header.message_length = 0;
  response.header.magic_cookie = kMagicCookie;
  std::memcpy(response.header.transaction_id, request.header.transaction_id,
              sizeof(response.header.transaction_id));
...
void StunServer::send_message(const stun::StunMessage &message,
  ...
  const ssize_t sent_size = sendto(m_socket_fd, response_buffer.data(), response_size, 0,
...
std::string StunServer::errno_message(const std::string &message, int error_number) {
