...
void StunClient::send(std::span<const uint8_t> data) {
  ...
  ssize_t ret = sendto(m_socket_fd, data.data(), data.size(), 0, (sockaddr *)&servaddr,
                       sizeof(servaddr));
  if (ret < 0) {
...
void StunClient::send(const StunMessage& message) {
  sockaddr_in servaddr{};
  bzero(&servaddr, sizeof(servaddr));
  servaddr.sin_family = AF_INET;
  ...
  ssize_t ret = sendto(m_socket_fd, data, size, 0, (sockaddr *)&servaddr,
                       sizeof(servaddr));
  if (ret < 0) {
    throw std::runtime_error(errno_message("error during sending data", errno));
  }
...
StunMessage StunClient::receive() {
  uint8_t buffer[kBufferSize];

  struct sockaddr response_addr{};
  socklen_t response_addr_len = sizeof(response_addr);
  int flags = 0;
  ssize_t received_n_bytes = recvfrom(m_socket_fd, buffer, sizeof(buffer), flags,
...
void StunClient::create_client_socket() {
